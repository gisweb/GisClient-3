...
switch ($fieldName) {
    case "stato":
        $fieldIdName = "idstatosegnalazione";
        break;
    case "tipo":
        $fieldIdName = "idtiposegnalazione";
        break;
    case "comune":
        $fieldIdName = "istatcomune";
        if(!empty($_REQUEST["bacino"])) array_push($filters, " and  idbacino = ".$_REQUEST["bacino"]);
        break;
    case "bacino":
        $fieldIdName = "idbacino";
        break;    
    case "tipomanutenzione":
        $fieldIdName = "idtipomanutenzione";
        break;    
}
...
if(!empty($field["relation_id"])) {//il campo oggetto di autosuggest è su tabella secondaria
    ...
    $sqlQuery = "SELECT DISTINCT ". $fieldIdName ." as id, ". $fieldName ." as text FROM " .$joinString;
} else { //caso elementare: il campo è su tabella del layer
    $sqlQuery = "SELECT DISTINCT ". $fieldIdName ." as id, ". $fieldName ." as text FROM " . $field["schema"].".". $field["table_name"] ." as " .DATALAYER_ALIAS_TABLE;
}

$sqlQuery .= ' where ' . $fieldName .' is not null ';


if(!empty($params)) {
    array_push($filters, ' and ' . $fieldName . ' ilike :input_string ');
}

if(!empty($filters)) {
    $sqlQuery .= implode(' and ', $filters);
}
