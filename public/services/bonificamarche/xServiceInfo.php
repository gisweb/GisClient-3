...
if(!isset($_REQUEST["request"])){
	$result=array("success"=>"ko","message"=>"Manca il parametro request");
	toJSON($result);
}
if(!isset($_REQUEST["dataschema"])){
	$_REQUEST["dataschema"]="segnalazioni";
	//$result=array("success"=>"ko","message"=>"Manca lo schema per i dati");
	//toJSON($result);
}
$dataSchema = $_REQUEST["dataschema"];
$dbSchema = DB_SCHEMA;
...
elseif($_REQUEST["request"]=="getdata"){
	$sql = "select colore,st_astext(the_geom),descrizione from ".$dataSchema.".annotazioni where segnalazione=".$_REQUEST["id"].";";
	$stmt = $db->prepare($sql);
	$stmt->execute();
	$row = $stmt->fetchAll(PDO::FETCH_NUM);
	$result=array("success"=>"ok","results"=>$row);
...
elseif($_REQUEST["request"]=="savedata"){
	...
	$sql = "update ".$dataSchema.".segnalazioni set the_geom = :geom where id=:id;";
	$stmt = $db->prepare($sql);
	$res = $stmt->execute(array(':geom'=>$_REQUEST["geom"],":id"=>$_REQUEST["id"])); 
	if($stmt->rowCount() == 0){
		$sql = "insert into ".$dataSchema.".segnalazioni (idstatosegnalazione,id,the_geom) values ('1',:id,:geom);";
		$stmt = $db->prepare($sql);
		$stmt->execute(array(':geom'=>$_REQUEST["geom"],":id"=>$_REQUEST["id"])); 
	}

	if(isset($_REQUEST["overlays"])){
		$sql = "delete from ".$dataSchema.".annotazioni where segnalazione=".$_REQUEST["id"].";";
		$stmt = $db->prepare($sql);
		$stmt->execute(); 
		for($i=0;$i<count($_REQUEST["overlays"]);$i++){
			$overlay = $_REQUEST["overlays"][$i];
					if(!isset($overlay["description"])) $overlay["description"] = '';
			$sql = "insert into ".$dataSchema.".annotazioni (segnalazione,colore,descrizione,the_geom) values (".$_REQUEST["id"].",'".$overlay["color"]."','".str_replace("'", "''", $overlay["description"])."','".$overlay["geom"]."');";
			$stmt = $db->prepare($sql);
			$stmt->execute(); 
		}
	}
